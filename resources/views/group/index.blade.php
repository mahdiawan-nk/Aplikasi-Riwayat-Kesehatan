                ...
                <div class="ms-auto">
                    <div class="btn-group" id="list-action-btn">
                        <button type="button" class="btn btn-primary btn-sm" id="tambah-data-karyawan"
                            onclick="tambahDataPermissions()"> <i class="bx bx-plus"></i>Tambah Group User</button>
                        ...
                        <div class="card-body">
                            <ul class="list-group">
                                <a href="{{ url('/panel-admin/managemen-user') }}"
                                    class="list-group-item list-group-item-action" aria-current="true">Pengguna</a>
                                <a href="{{ url('/panel-admin/group-user') }}"
                                    class="list-group-item list-group-item-action active" aria-current="true">Group</a>
                            </ul>
                        </div>
                    </div>
                </div>
...
@section('script')
    ...
    <script>
        ...
        const BaseUrlApi = "{{ url('panel-admin/group-user/roles') }}";
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: (value, item) => {
                        ...
                        <a href="javascript:;" onclick="editDataPermissions(${item.id})"><i class="bx bx-edit-alt"></i></a>
                        <a href="javascript:;" class="ms-4 bg-danger text-white" onclick="deleteDataPermissions(${item.id})"><i class="bx bx-trash-alt"></i></a>
                    </div>`
                    }
                }
        ...
        const insertDataPermissions = async () => {
            const selectedPermissions = $('#list-permission input[type="checkbox"]:checked')
                .map(function() {
                    return $(this).val(); // Get the value attribute which is the ID
                }).get();
            DataPermission.name = $('#name').val();
            DataPermission.permission = selectedPermissions;
            try {
                const response = await axios.post(BaseUrlApi, DataPermission, headers);
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    title: "Success " + response.data.message,
                    text: "Data Permission berhasil disimpan.",
                    icon: "success",
        ...
        const updateDataPermissions = async () => {
            const selectedPermissions = $('#list-permission input[type="checkbox"]:checked')
                .map(function() {
                    return $(this).val(); // Get the value attribute which is the ID
                }).get();
            DataPermission.name = $('#name').val();
            DataPermission.permission = selectedPermissions;
            DataPermission._method = 'PUT';
            headers.headers.Authorization = `Bearer ${token}`
            headers.headers.Accept = 'application/json';
            console.log(uidData)
            try {
                const response = await axios.post(BaseUrlApi + '/' + uidData, DataPermission, headers);
                Swal.fire({
                    toast: true,
                    position: "top-end",
                    title: "Success " + response.data.message,
        ...
        const editDataPermissions = async (id) => {

            try {
                const response = await axios.get(BaseUrlApi + '/' + id, headers);
                const data = response.data.data;
                uidData = data.role.id;

                $('#name').val(data.role.name)
                listPermissions(data.rolePermissions);
                modeForm = 'update';
                pageListData.hide()
                listBtnAction.hide()
                pageFormData.show()
        ...
        const deleteDataPermissions = async (id) => {
            Swal.fire({
                title: "Are you sure?",
        ...
        const listPermissions = async (data = []) => {
            ...
            try {
                const response = await axios.get('{{ url('/panel-admin/group-user/permissions') }}', headers);
                const permissions = response.data.data;
                let viewList = permissions.map((permission) => {
                    const isChecked = data.includes(permission.id) ? 'checked' : '';
                    return `<div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" role="switch" name="permissions[]" ${isChecked} value="${permission.id}" id="permission-${permission.id}">
                                <label class="form-check-label" for="permission-${permission.id}">${permission.name}</label>
                            </div>`
                }).join('')

                $('#list-permission').empty().append(viewList);
            } catch (error) {
                console.log(error)
            }
        }
